...
//Declares the Motor Controllers using their CAN Id as parameters
  ...
  public void move (double l, double r, double leftspeed, double rightspeed){
    m_auto.resetEncoders();

    double left = getLeft()+l;
    double right = getRight()+r;

    int x = 0;
    tankdrive.tankDrive(-leftspeed, -rightspeed);
    while (Math.abs(getLeft())  < Math.abs(left) || Math.abs(getRight()) < Math.abs(right) ){
      tankdrive.tankDrive(-leftspeed, -rightspeed);
